...
\lemma{Zij $F$ een willekeurig lichaam, dan is er een unieke afbeelding $\alpha: \mathbb{Q}_0 \to F$ zdd $\alpha(0) = 0, \alpha(1) = 1, \alpha(m + n) = \alpha(m) + \alpha(m)$}
\bewijs{
	$\alpha(0)=0, \alpha(1)=1 \alpha(s(x)) = \alpha(x) + 1$\\*
	Deze afbeelding voldoet automatisch aan: \\*$\alpha(x + y) = \alpha(x) + \alpha(y)$
}

\definitie{Als $\alpha: \mathbb{N}_0 \to F$ injectief is dan heeft $F$ karakteristiek nul}{
	Dus: $\mathbb{N}_0 \subseteq F$
}

\feit{Elk geordend lichaam heeft karakteristiek nul}
\bewijs{
$\alpha(0)=0, \alpha(1)=1 \alpha(s(x)) = \alpha(x) + 1$\\
$n \in \mathbb{N}_0: \alpha(n + 1) = \alpha(s(n)) = \alpha(n) + 1 > \alpha(n)$\\
$\alpha(0) < \alpha(1) < \alpha(2) ...$\\
Dus $n \neq m \Leftrightarrow \alpha(n) \neq \alpha(m)$ dus injectie
}

\stelling{$\alpha: \mathbb{N}_0 \to F$ kunnen we uitbreiden naar $\alpha: \mathbb{Z} \to F$}
\uitwerking{
	$(m,n)\in \mathbb{Z}$\\
...
\stelling{$\alpha: \mathbb{Z} \to F$ kunnen we uitbreiden naar $\alpha: \mathbb{Q} \to F$}
\uitwerking{
...
\definitie{Een geordend lichaam heet Archimedis}{
	\als{$\forall x \in F  \exists n \in \mathbb{N}$ zdd $x \leq \alpha(n)$}
}
...
\als{$x > 0$}\\
\uitwerking{
	Neem $j = \{\frac{1}{n}|n\in\mathbb{N}\}$\\
	Duidelijk: $0$ is een ondergrens voor $j$ want\\ $\forall n \in \mathbb{N}: 0 \leq \frac{1}{n}$\\
	...
	\uitwerking{
		...
		$\exists n \in \mathbb{N}$ zdd $\frac{1}{n} < 2l \Rightarrow J \ni \frac{1}{2n} < l$\\
		Tegenspraak dus: $l = 0$
	}\\
	\als{$x > 0$}\\
	\uitwerking{
	...
	$j$ (want 	$inf(J)=0$\\
	dus $\exists n\in \mathbb(n)$\\
	$\frac{1}{n} < \frac{1}{x}$ dus $n > x$
	}
}
}

\stelling{Zij $F$ volledig geordend lichaam: $x,y \in F \wedge x < y: \exists r \in \mathbb{Q} $ zdd $x < r < y$}
\bewijs{
	\als{$x < 0 < y: r = 0$}\\
	\als{$x < y \leq 0 \Rightarrow 0 \leq -y < -x$}\\
	\uitwerking{
		Stel de uitspraak is bewezen voor\\ $0 \leq x \leq y \Rightarrow -y < r < -x\Rightarrow x < -r < y$\\
		Stel $0 \leq x < y \Rightarrow \exists n \in \mathbb{N}$ zdd $\frac{1}{n} < y - x$.(archimedis)\\
		Def $A = \{k \in \mathbb{N}_0|k \leq nx\} \neq \emptyset$ (archimedis)\\
		Def $a = max(A) \in \mathbb{N}_0$: nu $a \leq nx < a + 1$\\
		$\frac{a}{n} \leq x < \frac{a+1}{n=r}$\\
		$r = \frac{a}{n} + \frac{1}{n} < x + (y-x) < y$
	}
}

\propositie{Zij $F$ volledig geordend lichaam: $x,y \in F \wedge x < y: \exists r \in \mathbb{Q} $ zdd $x < r < y$}
\bewijs{
	\als{$x < 0 < y: r = 0$}\\
	\als{$x < y \leq 0 \Rightarrow 0 \leq -y < -x$}\\
	\uitwerking{
